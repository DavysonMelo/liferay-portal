...
test('Can delete permanently multiple contents from Recycle Bin', async ({
	apiHelpers,
	contentsPage,
	page,
	recycleBinPage,
	...
	await test.step('Delete the contents so it goes into the Recycle Bin', async () => {
		await contentsPage.goto();
	...
	await test.step('Go to the Recycle Bin and delete the contents permanently', async () => {
		...
		await expect(page.getByRole('cell', {name: contentName1})).toBeHidden();
		await expect(page.getByRole('cell', {name: contentName2})).toBeHidden();
	});
});

test('Can empty the Recycle Bin', async ({
	apiHelpers,
	contentsPage,
	page,
	recycleBinPage,
	...
	await test.step('Go to the Recycle Bin and empty it', async () => {
		...
		await page
			.getByRole('menuitem', {name: 'Empty Recycle Bin'})
			.waitFor({state: 'visible'});

		await page.getByRole('menuitem', {name: 'Empty Recycle Bin'}).click();
		...
		waitForAlert(
			page,
			'Success:2 items were permanently deleted from the recycle bin.'
		);
	});
});
